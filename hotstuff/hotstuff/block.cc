...
void
HotstuffBlock::write_to_disk(const char* data_dir) {
	...
	save_block(wire_block, data_dir);
	if (parent_block_ptr != nullptr) {
		parent_block_ptr -> write_to_disk(data_dir);
	}
}
...
block_ptr_t
HotstuffBlock::load_decided_block(Hash const& hash, const char* data_dir) {
	auto loaded = load_block(hash, data_dir);
	if (!loaded) {
